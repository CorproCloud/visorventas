    ...
    async (files: FileList | File[]) => {
      ...
      try {
        for (const file of arr) {
          const { records, periods } = await parseFile(file);
          if (records.length === 0) {
            setStatus({ type: "err", msg: `${file.name}: no se detectaron filas válidas.` });
            continue;
          }
          addDataset(file.name.replace(/\.[^.]+$/, ""), records, periods);
        }
        setStatus({ type: "ok", msg: `${arr.length} archivo(s) procesado(s) correctamente.` });
        onComplete?.();
      } catch (e) {
    ...
    <div>
      ...
      >
        ...
        <div className="text-sm font-semibold text-foreground">
          {busy ? "Procesando..." : "Arrastra o haz clic para subir"}
        </div>
